...
[2026-05-26] [GAP]     W7/W8         PromptUtils.applyGlobalPrompt logcat trace not directly verified — UI tap challenge above. Code path structurally verified. For future verification, either (a) switch to debug build and inject chat via DebugTaskReceiver `--es task ...`, or (b) one human tap on Send with the current signed-release state on Pixel.
