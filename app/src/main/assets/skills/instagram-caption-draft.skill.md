...
tools: open_app, get_screen_info, take_screenshot, input_text, finish
...
3. If Instagram is visible, use get_screen_info first to read text, buttons, input fields, and the current UI tree. Use take_screenshot only when the relevant context is image-only or missing from the Accessibility tree. Do not collect profiles, hidden data, followers, or private content.
4. Draft three caption options: short, medium, and sales-focused. Include one clear CTA and 3-8 relevant hashtags.
5. If the user asks to prepare the post in Instagram, open Instagram, navigate only as far as the caption field, and use input_text with the node_id of the most direct visible UI field from the tree.
...
→ get_screen_info()
→ take_screenshot() only if the post context is visual-only
...
- If the user asks for spam, fake engagement, data collection, or mass posting: refuse and offer a manual content-drafting alternative.
