...
tools: open_app, get_screen_info, swipe, finish
...
3. Open Instagram only if requested. Use get_screen_info to read visible comments from the Accessibility tree. Use swipe at most 3 times unless the user explicitly asks to continue.
...
→ get_screen_info()
→ swipe(...) if more visible comments are needed
