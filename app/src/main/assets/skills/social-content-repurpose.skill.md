...
3. If content is visible on screen, use get_screen_info first to read visible text from the Accessibility tree. Use take_screenshot only when the source is image-only or the UI tree is incomplete.
...
→ get_screen_info()
→ take_screenshot() only if the listing is image-only
